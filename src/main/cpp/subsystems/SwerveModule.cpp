...
//#endif
  ...
  m_turningMotor.ConfigPeakOutputReverse(-0.5, kTimeoutMs);
...
//----------WDR: Need to check this is returning correct data.  Added for new Pose Estimator--------

frc::SwerveModulePosition SwerveModule::GetPosition()
{
  return {units::meter_t{m_driveMotor.GetSelectedSensorPosition() * ModuleConstants::kDriveEncoderMetresPerPulse},
          units::radian_t{m_turningMotor.GetSelectedSensorPosition() * ModuleConstants::kTurningEncoderRadiansPerPulse}};
}
...
void SwerveModule::SetDesiredState(const frc::SwerveModuleState &referenceState)
{
  ...
  const auto state =
      frc::SwerveModuleState::Optimize(referenceState, frc::Rotation2d(GetSwerveTurningFalconInternalRadians()));
  ...
  */
  ...
  if (referenceState.speed.value() > 0.09) {
    state=frc::SwerveModuleState::Optimize(referenceState, frc::Rotation2d(GetSwerveTurningFalconInternalRadians()));
 ...
 // m_driveMotor.Set(ControlMode::Velocity, speedInUnitsPer100mS);
  ...
  if (m_prevModuleAngleDegrees > 90)
  {
    ...
    if (state.angle.Degrees().value() < -90)
    {
      ...
      //fmt::print("DELTA ANGLE COUNTER-CW EXCEEDS 180 prev:{} desired:{} encoderRevs++ {}\n",m_prevModuleAngleDegrees,state.angle.Degrees().value(), m_encoderWindUpRevolutions);
    }
  }
  ...
  if (m_prevModuleAngleDegrees < -90)
  {
    ...
    if (state.angle.Degrees().value() > 90)
    {
      ...
      m_encoderWindUpRevolutions--;    
      //fmt::print("DELTA ANGLE CLOCKWISE EXCEEDS 180 prev:{} desired:{} encoderRevs-- {}\n",m_prevModuleAngleDegrees,state.angle.Degrees().value(), m_encoderWindUpRevolutions);
    }
  }
...
units::radian_t SwerveModule::GetSwerveTurningFalconInternalRadians()
{
  ...
  return units::radian_t(remainder((m_turningMotor.GetSelectedSensorPosition() * ModuleConstants::kTurningEncoderRadiansPerPulse), 360.0));
    ...
    void SwerveModule::SetParkMode()
    {
        ...
        SetDesiredState(frc::SwerveModuleState{0_mps,frc::Rotation2d(units::angle::degree_t(m_brakeAngle))});
    }
